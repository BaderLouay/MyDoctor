...
public class HealthArticlesActivity extends AppCompatActivity {
    ...
    private String [][] articles_details =
            {
                    {"Walking Daily", "", "", "", "Click for more details"},
                    {"Home care from COVID-19", "", "", "", "Click for more details"},
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        list = new ArrayList();
        for(int i=0;i<articles_details.length;i++){
            item = new HashMap<String,String>();
            item.put( "line1", articles_details[i][0]);
            item.put( "line2", articles_details[i][1]);
            item.put( "line3", articles_details[i][2]);
            item.put( "line4", articles_details[i][3]);
            item.put( "line5", articles_details[i][4]);
            list.add(item);
        }
        ...
        lst.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int i, long l) {
                ...
                it.putExtra("text1",articles_details[i][0]);
